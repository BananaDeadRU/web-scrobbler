...
function setupNewConnector() {
	...
	Connector.getTrack = () => {
		...
		if (!playerContainer) {
			console.warn(
				'[Yandex Connector getTrack] The playerContainer is missing',
			);
			return null;
		}
		...
		if (!infoContainer) {
			console.warn(
				'[Yandex Connector getTrack] The infoContainer is missing',
			);
			return null;
		}
		...
		if (!titleContainer) {
			console.warn(
				'[Yandex Connector getTrack] The titleContainer is missing',
			);
			return null;
		}
		...
		if (!link) {
			console.warn(
				'[Yandex Connector getTrack] The link inside the titleContainer was not found.',
			);
			return null;
		}
		...
		if (!titleSpan) {
			console.warn(
				'[Yandex Connector getTrack] The span with the track name was not found.',
			);
			return null;
		}
	...
	Connector.getArtist = () => {
		...
		if (!playerContainer) {
			console.warn(
				'[Yandex Connector getArtist] There is no playerContainer',
			);
			return null;
		}
		...
		if (!artistContainer) {
			console.warn(
				'[Yandex Connector getArtist] The artistContainer was not found',
			);
			return null;
		}
		...
		const span = firstLink.querySelector(
			'span[class*="Meta_artistCaption"]',
		);
		if (!span) {
			console.warn(
				'[Yandex Connector getArtist] There is no span with artist name',
			);
			return null;
		}
